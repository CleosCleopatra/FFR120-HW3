...
#ad
delta_arr = np.full(N_part, delta_neg)
delta_arr[:N_pos] = delta_pos
...
traj_pos_x_plot = traj_pos_x.copy()
traj_pos_y_plot = traj_pos_y.copy()

# Detect large jumps (crossing boundary)
jump_idx = np.where(np.abs(np.diff(traj_pos_x)) > L/2)[0]
mask = np.zeros_like(traj_pos_x_plot, dtype=bool)
mask[jump_idx + 1] = True

traj_pos_x_masked = np.ma.masked_where(mask, traj_pos_x_plot)
traj_pos_y_masked = np.ma.masked_where(mask, traj_pos_y_plot)
...
plt.plot(traj_pos_x_masked, traj_pos_y_masked, label= 'neg-delay trajectory')
plt.scatter(traj_pos_x[0], traj_pos_y[0], marker='x', c='green', label='neg start')
plt.scatter(traj_pos_x[-1], traj_pos_y[-1], marker='x', c = 'red', label = 'neg end')
